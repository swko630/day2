/*
   ...
   4. 종류 
     ...
     CONSTRAINT PK_BANK PRIMARY KEY(BANK_CODE)--PRIMARY KEY 제약조건 이름지어주기
     );
...
/*
   ...
   3. 칼럼 삭제하기      : ALTER TABLE 테이블명 DROP COLUME    칼럼명
   4. 칼럼 이름 바꾸기   : ALTER TABLE 테이블명 RENAME COLUME 기존칼럼명 TO 신규칼럼명 
   5. 테이블 이름 바꾸기 : ALTER TABLE 테이블명 RENAME TO     신규테이블명
   */
   
-- 1. BANK_T에 연락처(BANK_TEL) 칼럼을 추가하시오.
 ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(30 BYTE);
...
ALTER TABLE CUSTOMER_T RENAMED COLUMN CUST_PHONE TO CUST_TEL;

-- 5. CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오. 'VIP','GOLD','SLIVER','BRONZE'중 하나의 값을 가지도록 설정하시오.
ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE = 'VIP' OR 'GOLD' OR 'SLIVER'OR'BRONZE');
                                            --   =CHECK(GRADE IN('VIP','GOLD','SLIVER','BRONZE'));

-- 6. CUSTOMER_T의 고객명(CUST_NAME) 칼럼의 필수 제약조건을 없애시오.
ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --ALTER는 입력한 그대로 바꿔주겠다는거기 때매 NULL 생략하면 안됨
